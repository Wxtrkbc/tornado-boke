...
class Pagenation:
    ...
    def generate_str_page(self, base_url):
        list_page = []
        ...
        if self.current_page == 1:   # 上一页
            ...
            prev = '<li><a class="pre-page" href="%s%s">上一页</a></li>' % (base_url, self.current_page - 1,)
        list_page.append(prev)

        for p in range(start_page, end_page):  # 1-10
            if p == self.current_page:
                temp = '<li><a class="active" href="%s%s">%s</a></li>' % (base_url, p, p)
            else:
                temp = '<li><a href="%s%s">%s</a></li>' % (base_url, p, p)
            list_page.append(temp)

        if self.current_page == self.all_pages:  # 下一页
            nex = '<li><a class="next-page" href="javascript:void(0);">下一页</a></li>'
        else:
            nex = '<li><a class="next-page" href="%s%s">下一页</a></li>' % (base_url, self.current_page + 1,)
        list_page.append(nex)

        # # 跳转
        # jump = """<input type='text' /><a onclick="Jump('%s',this);">GO</a>""" % ('/contents/')
        # script = """<script>
        ...
        page_info = '<div class="page-info"><span>{}/{} 页</span></div>'.format(self.current_page, self.all_pages)
        return str_page+page_info