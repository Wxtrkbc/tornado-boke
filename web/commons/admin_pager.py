...
class Pagenation:
    ...
    def generate_str_page(self, ):
        ...
        if self.current_page == 1:  # 上一页
            ...
            prev = '<li><a href="javascript:void(0);" onclick="ChangePage(%s)">上一页</a></li>' % (
                self.current_page - 1,)
        list_page.append(prev)

        for p in range(start_page, end_page):  # 1-10
            if p == self.current_page:
                temp = '<li class="active"><a href="javascript:void(0);" ' \
                       'onclick="ChangePage(%s)">%s</a></li>' % (p, p)
            else:
                temp = '<li><a href="javascript:void(0);" onclick="ChangePage(%s)">%s</a></li>' % (
                    p, p)
        ...
        if self.current_page == self.all_pages:  # 下一页
            ...
            nex = '<li><a   href="javascript:void(0); "onclick="ChangePage(%s)">下一页</a></li>' % (
                self.current_page + 1,)
        list_page.append(nex)

        first_page = '<li><a href="javascript:void(0);" onclick="ChangePage(1)">首页</a></li>'
        last_page = ' <li><a href="javascript:void(0);" onclick="ChangePage(%s)">尾页</a></li>' % (
            self.all_pages)
        str_page = "".join(list_page)
        page_info = '<li><a class="page-info"><span>{}/{} 页</span></a></li>'.format(
