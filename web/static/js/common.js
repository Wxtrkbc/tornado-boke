...
(function(jq){
    ...
    function ExecuteTdIntoEdit($td, specialInEditFunc){
        ...
        if(editType=='input'){
            ...
            if(specialInEditFunc){
                ...
                var text = $td.text();
                $td.addClass('padding-3');
                var htmlTag = $.CreateSelect({'class':'padding-tb-5 form-control','onchange':'MultiSelect(this)'}, {'width':'100%'}, window[options], text, value_key,text_key);
                $td.empty().append(htmlTag);
            }
        }
	...
	jq.extend({
         ...
         */
        'CreateSelect': function(attrs,csses,options,current,key_value,key_text){
        ...
        'BindDoSingleCheck':function(tableBody, specialInEditFunc, specialOutEditFun){
            $(tableBody).find(':checkbox').bind('click',function(){
                var $tr = $(this).parent().parent();
